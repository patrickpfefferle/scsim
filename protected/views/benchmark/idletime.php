<?php
$this->pageTitle = Yii::app()->name . ' - ' . Yii::t('app', 'Benchmark of group idle time');

$this->widget('application.components.amai.AmaiPageHeader', array(
    'header' => Yii::t('app', 'Idle time'),
    'subtitle' => Yii::t('app', 'Benchmark of group idle time')
));
...
for ($i = 0; $i <= count($groups) - 1; $i++) {
    ...
    for ($f = 0; $f <= count($simresults) - 1; $f++) {
        $results[$i]['data'][$f] = round($simresults[$f]->idle_time, 0);
    }
}
