<?php
$this->pageTitle = Yii::app()->name . ' - ' . Yii::t('app', 'Benchmark of group capacity ratio');

$this->widget('application.components.amai.AmaiPageHeader', array(
    'header' => Yii::t('app', 'Capacity ratio'),
    'subtitle' => Yii::t('app', 'Benchmark of group capacity ratio')
));
...
for ($i = 0; $i <= count($groups) - 1; $i++) {
    ...
    for ($f = 0; $f <= count($simresults) - 1; $f++) {
        $results[$i]['data'][$f] = round($simresults[$f]->capacity_ratio, 2);
    }
}
