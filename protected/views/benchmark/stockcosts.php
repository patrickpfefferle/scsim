<?php
$this->pageTitle = Yii::app()->name . ' - ' . Yii::t('app', 'Benchmark of group storage costs');

$this->widget('application.components.amai.AmaiPageHeader', array(
    'header' => Yii::t('app', 'Storage costs'),
    'subtitle' => Yii::t('app', 'Benchmark of group storage costs')
));
...
for ($i = 0; $i <= count($groups) - 1; $i++) {
    ...
    for ($f = 0; $f <= count($simresults) - 1; $f++) {
        $results[$i]['data'][$f] = round($simresults[$f]->storage_costs, 2);
    }
}
