<?php
$this->pageTitle = Yii::app()->name . ' - ' . Yii::t('app', 'Benchmark of group sales');

$this->widget('application.components.amai.AmaiPageHeader', array(
    'header' => Yii::t('app', 'Sales'),
    'subtitle' => Yii::t('app', 'Benchmark of group sales')
));
...
for ($i = 0; $i <= count($groups) - 1; $i++) {
    ...
    for ($f = 0; $f <= count($simresults) - 1; $f++) {
        $results[$i]['data'][$f] = round($simresults[$f]->sales_quantity, 0);
    }
}
