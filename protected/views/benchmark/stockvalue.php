<?php
$this->pageTitle = Yii::app()->name . ' - ' . Yii::t('app', 'Benchmark of group stock value');

$this->widget('application.components.amai.AmaiPageHeader', array(
    'header' => Yii::t('app', 'Stock Value'),
    'subtitle' => Yii::t('app', 'Benchmark of group stock value')
));
...
for ($i = 0; $i <= count($groups) - 1; $i++) {
    ...
    for ($f = 0; $f <= count($simresults) - 1; $f++) {
        $results[$i]['data'][$f] = round($simresults[$f]->stock_value, 0);
    }
}
